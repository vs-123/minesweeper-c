![minesweeper -- vs-123 -- screenshot](https://github.com/vs-123/minesweeper-c/blob/main/assets/screenshot.png)

# MINESWEEPER

  A simple minesweeper implementation in C.

  This a C port of my original C++ Minesweeper project @ https://github.com/vs-123/minesweeper

## FEATURES
- [x] Fun
- [x] Reveal Cells
- [x] Game Over/Win screen
- [x] Flags
- [x] Auto Reveal Cells with Middle Click
- [x] Timer
- [x] Icons
- [x] Coloured Numbers

## CONTROLS

- LMB / F       -- Clear a Cell
- RMB / [SPACE]	-- Flag a cell
- MMB / E       -- Auto reveal

## BUILD INSTRUCTIONS

- Clone the repository recursively and `cd` into it:
```bash
% git clone --recursive https://github.com/vs-123/minesweeper-c.git
% cd minesweeper-c/
```

- Use CMake to build:
```bash
% cmake -B build/ -S .
% cmake --build build/
% ./build/mswpr # run minesweeper
```
   
## LICENSE

  This project is licensed under **GNU Affero General Public License Version 3.0 or Later**.
  
  **NO WARRANTY PROVIDED!**
  
  See **LICENSE** file for full terms or visit https://www.gnu.org/licenses/agpl-3.0.html
