...
mswpr_t mswpr_init(void) {
   ...
   InitWindow(mswpr.screen_width, mswpr.screen_height, "Minesweeper -- vs-123");
         ...
         for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
               ...
               if (new_row >= 0 && new_row < mswpr->grid_rows && new_col >= 0 &&
                  ...
                  cell_t *neighbor =
                      &mswpr
                           ->cell_grid[new_row * mswpr->grid_columns + new_col];
                  if (neighbor->is_mine) {
                     count++;
                  }
               }
   ...
   if (mswpr->is_reset_confirmed) {
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
         if (CheckCollisionPointRec(mouse_position, mswpr->btn_yes)) {
            ...
            mswpr->pause_duration += GetTime() - mswpr->confirm_start_time;
            mswpr->is_reset_confirmed = false;
         }
      }
   } else {
      if (!mswpr->has_game_ended && !mswpr->has_user_won) {
         if (mouse_position.y > mswpr->ui_height) {
            ...
            int cell_row =
                (int)(mouse_position.y - mswpr->ui_height) / mswpr->cell_size;

            if (cell_row >= 0 && cell_row < mswpr->grid_rows && cell_col >= 0 &&
                cell_col < mswpr->grid_columns) {
               cell_t *current_cell =
                   &mswpr->cell_grid[cell_row * mswpr->grid_columns + cell_col];

               if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) ||
                   IsKeyPressed(KEY_F)) {
                  if (mswpr->is_first_click) {
                     mswpr_first_click_safe_zone(mswpr, cell_row, cell_col);
                     mswpr->is_first_click = false;
                     current_cell =
                         &mswpr->cell_grid[cell_row * mswpr->grid_columns +
                                           cell_col];
                  }

                  if (!current_cell->has_flag)
                     mswpr_reveal_cell(mswpr, cell_row, cell_col);
               } else if (IsMouseButtonPressed(MOUSE_RIGHT_BUTTON) ||
                          IsKeyPressed(KEY_SPACE)) {
                  if (!current_cell->is_revealed)
                     current_cell->has_flag = !current_cell->has_flag;
               } else if (IsMouseButtonPressed(MOUSE_MIDDLE_BUTTON) ||
                          IsKeyPressed(KEY_E)) {
                  if (current_cell->is_revealed &&
                      current_cell->adjacent_mines_count > 0)
                     mswpr_auto_reveal(mswpr, cell_row, cell_col);
               }
            }
         }

         if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
             CheckCollisionPointRec(mouse_position, mswpr->btn_new_game)) {
...
void mswpr_draw_cell(mswpr_t *mswpr, int row, int col) {
   ...
   if (cell.is_revealed) {
      if (cell.is_mine) {
         if (mswpr->mine_texture.id > 0) {
            float scale = (float)mswpr->cell_size / mswpr->mine_texture.width;
            DrawTextureEx(mswpr->mine_texture, (Vector2){(float)x, (float)y},
                          0.0f, scale, WHITE);
         } else {
            DrawText("M", x + mswpr->cell_size / 4, y + mswpr->cell_size / 4,
                     20, BLACK);
         }
      } else if (cell.adjacent_mines_count > 0) {
         int num          = cell.adjacent_mines_count;
         const char *text = TextFormat("%d", num);
         int centerX      = x + (mswpr->cell_size - MeasureText(text, 20)) / 2;
         int centerY      = y + (mswpr->cell_size - 20) / 2;
         DrawText(text, centerX, centerY, 20, mswpr->colour_palette[num]);
      }
   } else if (cell.has_flag) {
      if (mswpr->flag_texture.id > 0) {
         float scale = (float)mswpr->cell_size / mswpr->flag_texture.width;
         DrawTextureEx(mswpr->flag_texture, (Vector2){(float)x, (float)y}, 0.0f,
                       scale, WHITE);
      } else {
         DrawText("F", x + mswpr->cell_size / 3, y + mswpr->cell_size / 4, 20,
                  MAROON);
      }
   }
...
void mswpr_draw_ui(mswpr_t *mswpr) {
   ...
   DrawText(timer_txt, stats_x, stats_y, 20, (int)elapsed > 999 ? RED : BLACK);
   DrawText(mine_txt, stats_x + 130, stats_y, 20, MAROON);

   DrawLine(0, mswpr->ui_height - 1, mswpr->screen_width, mswpr->ui_height - 1,
            DARKGRAY);
}
...
void mswpr_draw_end_screen(mswpr_t *mswpr) {
   ...
   DrawRectangle(0, grid_y, mswpr->screen_width, grid_h, Fade(LIGHTGRAY, 0.8f));
...
void mswpr_draw_reset_modal(mswpr_t *mswpr) {
   ...
   DrawText("YES", mswpr->btn_yes.x + (mswpr->btn_yes.width / 2) - (yes_w / 2),
...
void mswpr_first_click_safe_zone(mswpr_t *mswpr, int safe_row, int safe_col) {
      ...
      for (int col = min_col; col <= max_col; col++) {
         ...
         if (current_cell->is_mine) {
            ...
            for (int r = 0; r < mswpr->grid_rows && !placed; r++) {
               for (int c = 0; c < mswpr->grid_columns && !placed; c++) {
                  if (r < min_row || r > max_row || c < min_col ||
                      c > max_col) {
                     cell_t *target =
                         &mswpr->cell_grid[r * mswpr->grid_columns + c];
                     if (!target->is_mine) {
                        target->is_mine = true;
                        placed          = true;
                     }
...
void mswpr_reveal_cell(mswpr_t *mswpr, int row, int col) {
...
void mswpr_auto_reveal(mswpr_t *mswpr, int row, int col) {
   cell_t *current_cell = &mswpr->cell_grid[row * mswpr->grid_columns + col];
   ...
   for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
         ...
         if (nr >= 0 && nr < mswpr->grid_rows && nc >= 0 &&
            ...
            if (mswpr->cell_grid[nr * mswpr->grid_columns + nc].has_flag)
               num_flagged++;
         }
      }
   }
